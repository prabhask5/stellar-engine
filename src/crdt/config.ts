 ...
 */
const DEFAULTS: ResolvedCRDTConfig = {
  ...
  localSaveDebounceMs: 5_000,
  cursorDebounceMs: 50,
  maxOfflineDocuments: 50,
  maxBroadcastPayloadBytes: 250_000,
  syncPeerTimeoutMs: 3_000,
...
let resolvedConfig: ResolvedCRDTConfig | null = null;
 ...
 * @internal
 */
export function _initCRDT(rawConfig: CRDTConfig, prefix: string): void {
  crdtPrefix = prefix;
  resolvedConfig = {
    ...
    maxBroadcastPayloadBytes:
      rawConfig.maxBroadcastPayloadBytes ?? DEFAULTS.maxBroadcastPayloadBytes,
 ...
 */
export function getCRDTConfig(): ResolvedCRDTConfig {
  if (!resolvedConfig) {
 ...
 */
export function getCRDTPrefix(): string {
  if (!resolvedConfig) {
    throw new Error('CRDT not configured. Add crdt to your initEngine() config.');
  }
  return crdtPrefix;
