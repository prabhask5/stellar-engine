...
export async function loadDocumentState(
  documentId: string
): Promise<CRDTDocumentRecord | undefined> {
  const db = getDb();
 ...
 */
export async function saveDocumentState(record: CRDTDocumentRecord): Promise<void> {
  const db = getDb();
  await db.table('crdtDocuments').put(record);
  debugLog(
    `[CRDT] Document ${record.documentId}: local save (${record.stateSize} bytes to IndexedDB)`
  );
}
 ...
 */
export async function deleteDocumentState(documentId: string): Promise<void> {
  const db = getDb();
 ...
 * @returns The document record, or `undefined` if not found.
 */
export async function loadDocumentByPageId(
  pageId: string
): Promise<CRDTDocumentRecord | undefined> {
  const db = getDb();
...
export async function appendPendingUpdate(documentId: string, update: Uint8Array): Promise<void> {
  const db = getDb();
  const record: CRDTPendingUpdate = {
    documentId,
    update,
 ...
 */
export async function loadPendingUpdates(documentId: string): Promise<CRDTPendingUpdate[]> {
  const db = getDb();
 ...
 */
export async function clearPendingUpdates(documentId: string): Promise<number> {
  const db = getDb();
 ...
 */
export async function isOfflineEnabled(documentId: string): Promise<boolean> {
 ...
 */
export async function getOfflineDocuments(): Promise<CRDTDocumentRecord[]> {
  const db = getDb();
 ...
 */
export async function getOfflineDocumentCount(): Promise<number> {
  const db = getDb();
