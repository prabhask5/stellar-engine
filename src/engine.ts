    ...
    if (createItems.length > 0) {
      ...
      for (const [tableName, items] of sortedTableEntries) {
        ...
        const payloads = items.map((item) =>
          filterPayloadToSchema(tableName, {
            id: item.entityId,
    ...
    if (batchableItems.length > 0) {
      ...
      for (const [tableName, items] of itemsByTable) {
        ...
        for (const item of items) {
          ...
          delete (rawPayload as Record<string, unknown>)._version;
    ...
    case 'increment': {
      ...
      if (!data) {
        ...
        if (localInc) {
          ...
          delete (rawInsertPayload as Record<string, unknown>)._version;
    ...
    case 'set': {
      ...
      if (!data) {
        ...
        if (localEntity) {
          ...
          delete (rawSetPayload as Record<string, unknown>)._version;
