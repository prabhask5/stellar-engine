 ...
 */
export async function extractCrdtText(docId: string): Promise<string> {
  ...
  try {
    const blocksArray = doc.getArray<Y.Map<unknown>>('blocks');
    const texts: string[] = [];

    for (const yMap of blocksArray.toArray()) {
      const content = yMap.get('content') as string | undefined;
      if (content) {
