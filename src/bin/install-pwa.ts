...
-->
<script lang="ts">
  import { isDemoMode, setDemoMode } from '@prabhask5/stellar-engine';

  const isActive = $derived(isDemoMode());
...
<div class="demo-page">
  <div class="demo-card">
    {#if isActive}
      ...
      <p>You're currently exploring ${opts.name} in demo mode. All data is sandboxed and will reset when you refresh the page.</p>
      <button class="demo-btn demo-btn-exit" onclick={exitDemo}>
        Exit Demo
      </button>
    {:else}
      <h1>Try ${opts.name}</h1>
      <p>Explore the app with sample data — no account needed. Everything runs in a sandboxed environment:</p>
      <ul>
        <li>All data resets on page refresh</li>
        <li>No account or email required</li>
        <li>Your real data is never touched</li>
      </ul>
      <button class="demo-btn demo-btn-start" onclick={startDemo}>
        Start Demo
      </button>
    {/if}
  </div>
